...
import lab2Baranauskas.VaisiuGamyba;
import lab2Baranauskas.Vaisius;
import lab2Baranauskas.GreitaveikosTyrimas;
import java.io.File;
...
public class Lab2WindowFX extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private static SortedSetADTx<Vaisius> autoSet;
    ...
    private void treeLeftLeaves()
    {
        KsFX.setFormatStartOfLine(true);
        BstSetKTUx set = new BstSetKTUx(new Vaisius());
        SetADT<Vaisius> vais = set.allLeftLeaves();      
        for (Vaisius a : vais)
        {
            KsFX.oun(taOutput, a.toString(), MESSAGES.getString("msg9"));
        }   
        KsFX.setFormatStartOfLine(false);
    }
    ...
    private void treeAdd() throws MyException {
        KsFX.setFormatStartOfLine(true);
        Vaisius vaisius = VaisiuGamyba.gautiIsKrepšio();
        autoSet.add(vaisius);
        paneParam1.getTfOfTable().get(2).setText(String.valueOf(--sizeOfLeftSubSet));   
        KsFX.oun(taOutput, vaisius, MESSAGES.getString("msg7"));
        KsFX.oun(taOutput, autoSet.toVisualizedString(tfDelimiter.getText()));
        KsFX.setFormatStartOfLine(false);
    }
    ...
    private void treeRemove() {
        ...
        if (autoSet.isEmpty()) {
            ...
            Vaisius vaisius = (Vaisius) autoSet.toArray()[nr];
            autoSet.remove(vaisius);
            KsFX.oun(taOutput, vaisius, MESSAGES.getString("msg6"));
            KsFX.oun(taOutput, autoSet.toVisualizedString(tfDelimiter.getText()));
        }
        KsFX.setFormatStartOfLine(false);
    }
    ...
    private void createTree() throws MyException {
        switch (cmbTreeType.getSelectionModel().getSelectedIndex()) {
            case 0:
                autoSet = new BstSetKTUx(new Vaisius());
                break;
            case 1:
                autoSet = new AvlSetKTUx(new Vaisius());
                break;
            default:
                disableButtons(true);
